...
struct __attribute__((__packed__)) IPv6Header {
    uint16_t versionAndTrafficClassAndFlowLabel;
    uint16_t payloadLength;
    uint8_t nextHeader;
    uint8_t hopLimit;
    unsigned char srcAddr[16];
    unsigned char dstAddr[16];
};
...
- (BOOL)validatePing6ResponsePacket:(NSMutableData *)packet sequenceNumber:(uint16_t *)sequenceNumberPtr {
    ...
    result = NO;
    NSInteger icmpSize = sizeof(*icmpPtr);
    
    if (packet.length >= icmpSize) {
        ...
        NSInteger ipv6HeaderSize = sizeof(*ipv6Header);
        if (packet.length >= icmpSize + ipv6HeaderSize)
        {
            ipv6Header = (struct IPv6Header *) (packet.mutableBytes + icmpSize);
            NSLog(@"%s",ipv6Header->dstAddr);
        }
